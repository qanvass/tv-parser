...
/// Movies presentation only. Consumes already-built rails — no catalog I/O.
class CinematicMoviesPage extends StatefulWidget {
  ...
  @override
  State<CinematicMoviesPage> createState() => _CinematicMoviesPageState();
}

class _CinematicMoviesPageState extends State<CinematicMoviesPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      MovieArtDebug.logRailSample(widget.rows);
  ...
  @override
  void didUpdateWidget(CinematicMoviesPage oldWidget) {
  ...
  List<TvStreamRecord> _continueWatching(WatchingState watch) {
    if (watch.movies.isEmpty) return const [];
    ...
    for (final w in watch.movies.take(12)) {
      ...
      if (matched != null) {
        ...
        out.add(_watchingFallback(w, progress));
      }
    }
    return out;
  }

  static TvStreamRecord _watchingFallback(WatchingModel w, double? progress) {
  ...
  bool _inMyList(FavoritesState favs, TvStreamRecord? rec) {
    ...
    return favs.movies.any((m) => m.streamId == id);
  }
  ...
  void _toggleMyList(TvStreamRecord rec) {
    ...
    final existing = cubit.state.movies.where((m) => m.streamId == id);
    final isAdd = existing.isEmpty;
    cubit.addMovie(
      ChannelMovie(
        name: rec.title,
  ...
  /// Presentation-only: keep the hero title from repeating across the first
  /// viewport rails when the row still has other titles. Catalog untouched.
  List<TvChannelRow> _dedupedRows(TvStreamRecord? hero) {
    final seen = <String>{};
    final heroId = hero?.streamId ?? hero?.enrichmentKey;
    if (heroId != null && heroId.isNotEmpty) seen.add(heroId);
    final out = <TvChannelRow>[];
    var index = 0;
    for (final row in widget.rows) {
      if (row.streams.isEmpty) continue;
      if (index == 0) {
        for (final s in row.streams) {
          final id = s.streamId ?? s.enrichmentKey;
      ...
      final filtered = <TvStreamRecord>[];
      for (final s in row.streams) {
        final id = s.streamId ?? s.enrichmentKey;
        if (id != null && id.isNotEmpty && seen.contains(id)) {
          if (row.streams.length > 1) continue;
        }
        if (id != null && id.isNotEmpty) seen.add(id);
        filtered.add(s);
      }
      if (filtered.isNotEmpty) {
        out.add(TvChannelRow(title: row.title, streams: filtered));
      }
      index++;
    }
    return out;
  ...
  Widget build(BuildContext context) {
    ...
    return ListenableBuilder(
      ...
      builder: (context, _) {
        ...
        final rows = _dedupedRows(_focused);
        return Column(
